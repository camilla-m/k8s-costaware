...
class Scheduler:
    ...
    def __init__(self, binary, cfg, kubeconfig):
        # O kube-scheduler IGNORA o flag --kubeconfig quando --config e dado:
        # a origem do client passa a ser clientConnection.kubeconfig, dentro do
        # proprio KubeSchedulerConfiguration. Os deploy/scenario-*.yaml sao
        # portateis (nao carregam um caminho de maquina), entao injetamos o
        # caminho aqui, em uma copia temporaria.
        with open(cfg) as fh:
            doc = yaml.safe_load(fh)
        doc.setdefault("clientConnection", {})["kubeconfig"] = os.path.abspath(kubeconfig)

        self.tmp = tempfile.NamedTemporaryFile(
            mode="w", suffix=".yaml", prefix="costaware-sched-", delete=False
        )
        yaml.safe_dump(doc, self.tmp)
        self.tmp.close()

        self.cmd = [binary, "--config", self.tmp.name, "--v=3"]
        self.proc = None
