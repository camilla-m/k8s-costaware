...
type CostAware struct {
	...
	// stateMu serializes only the get-or-create of the per-cycle state, not
	// the Phi computation. See getOrCreateState.
	stateMu sync.Mutex
}
...
// cycleState caches the raw Phi of each candidate node for the current
// scheduling cycle, so NormalizeScore can min-max over the feasible set.
//
// The framework fans Score out over the nodes in parallel goroutines that all
// share this one CycleState, so every access to raw must be guarded. Without
// the mutex the map writes race: Go can panic outright on a concurrent map
// write, and a lost write silently drops a node from raw, which NormalizeScore
// then scores 0 -- turning the winner into the loser at random.
...
// PreScore allocates the per-cycle state once, on the single-threaded path,
// before the framework fans Score out over the nodes. It is the fast path only:
// PreScore is a SEPARATE extension point, so a KubeSchedulerConfiguration that
// enables CostAware under `score` alone never calls it. getOrCreateState
// therefore must not depend on it having run.
func (c *CostAware) PreScore(_ context.Context, state *framework.CycleState, _ *v1.Pod, nodes []*framework.NodeInfo) *framework.Status {
	state.Write(stateKey, &cycleState{raw: make(map[string]float64, len(nodes))})
	return nil
}

// getOrCreateState returns the state for this scheduling cycle, creating it if
// PreScore did not.
//
// The lock is what makes it correct: CycleState.Read followed by
// CycleState.Write is not atomic, so without it two parallel Score goroutines
// can both miss the key and both Write a fresh map. Whichever one loses that
// Write keeps updating an orphaned map, and every node it scored is missing
// from raw when NormalizeScore runs -- which scores those nodes 0 and silently
// hands the cycle to the wrong node.
func (c *CostAware) getOrCreateState(state *framework.CycleState) (*cycleState, error) {
	c.stateMu.Lock()
	...
	if err == nil {
		...
		if !ok {
			return nil, fmt.Errorf("unexpected %s state type %T", Name, raw)
		}
		return st, nil
	}
