...
// TestCycleState_ConcurrentSetIsRaceFree guards the bug that crashed the whole
// scheduler process at scale: the framework fans Score out over every feasible
// node in parallel goroutines, and they all wrote the same map. Run with -race.
func TestCycleState_ConcurrentSetIsRaceFree(t *testing.T) {
	...
	for w := 0; w < workers; w++ {
		go func(w int) {
			defer wg.Done()
			for i := 0; i < 200; i++ {
				name := "node-" + strconv.Itoa(w) + "-" + strconv.Itoa(i)
				st.set(name, float64(w*i))
				if _, ok := st.get(name); !ok {
					t.Errorf("value written for %s not readable back", name)
				}
			}
		}(w)
	}
	...
	if got := len(st.raw); got != workers*200 {
		t.Errorf("expected %d entries, got %d", workers*200, got)
	}
}
